...
Ascoder is a privately deployable team code understanding platform. Users ask natural-language questions about a codebase; the system uses CodeGraph (a CLI tool) to analyze code structure and LLM agents (via AgentScope) to generate role-based answers. Currently in MVP phase.

- **Backend**：Java 17 / Spring Boot 3.3 / JPA / Flyway / MySQL 8.4 / Maven
- **Frontend**：Vue 3 / TypeScript / Pinia / Vue Router / Element Plus / Vite
- **Build**：Maven（backend），Vite（frontend），Docker Compose（deployment）

## Build & Run Commands

### Backend (Spring Boot 3.3.7 / Java 17 / Maven)

```bash
cd backend && ./mvnw package -DskipTests   # Build
./mvnw spring-boot:run                      # Run (requires MySQL and codegraph CLI on PATH)
./mvnw test                                  # Run all tests
./mvnw test -Dtest=DefaultPromptBuilderTests # Run a single test class
./mvnw test -Dtest="DefaultPromptBuilderTests#testBuildWithCodeContext" # Run a single test method
```
