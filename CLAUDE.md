...
异步框架、任务调度等内部基础框架模块，必须提供完善的 SPI（Service Provider Interface）供业务模块扩展和接入：

- **接口即契约**：SPI 接口必须有完整的 Javadoc，说明接口职责、生命周期、线程模型、调用时机和约束条件。每个方法必须注释其语义、参数含义、返回值约定和异常场景。
- **命名合理**：接口和方法命名应从调用方视角表达意图，而非暴露实现细节。例如 `TaskHandler` 优于 `AsyncRunnable`，`onCompleted(Result)` 优于 `callback(Result)`。
- **参数命名合理**：参数名必须表达业务语义，禁止使用 `arg0`、`param1`、`obj` 等无意义命名。参数类型优先使用对象而非 Map（参见"优先使用对象而非 Map 作为参数"规范）。
- **扩展点明确**：框架应通过接口 + 默认实现的方式提供扩展点，业务模块只需实现接口即可接入，无需了解框架内部机制。
...
能用类型化对象（class）表达参数时，禁止使用 `Map<String, Object>` 等无类型容器。对象参数提供编译期类型安全、IDE 自动补全和可读性；Map 只在动态结构（如 JSON 原始解析、第三方 API 响应透传）等无法预知字段时才允许使用。

### 消除魔法值

代码中禁止出现未命名的字面常量（魔法值）。数值、字符串等常量必须提取为 `private static final` 常量并赋予语义化命名。枚举能表达有限集合时优先使用枚举。例外：0/1 等数学含义明确的循环初始值、`""` 空字符串等无歧义值可内联使用。

### 及时删除未使用代码

不再被引用的类、方法、字段、导入、配置项必须及时删除，禁止以注释方式保留。版本控制系统已保存历史，无需在代码中留"墓碑"。
